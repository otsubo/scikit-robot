...
def sample_teleop():
    ...
    listener.waitForTransform('/vive_base','/right_controller',rospy.Time(), rospy.Duration(2.0))
    while not rospy.is_shutdown():
        (trans,quat) = listener.lookupTransform("/vive_base", "/right_controller", rospy.Time(0))
        target = skrobot.coordinates.Coordinates([trans[0],trans[1],trans[2]] ,
                                                 [quat[0],quat[1],quat[2],quat[3]])
        robot.rarm.inverse_kinematics(target, rotation_axis=True)
        ri.angle_vector(av=robot.angle_vector())
        viewer.redraw()
...
ri = skrobot.interfaces.ros.KinovaROSRobotInterface(robot=robot,
                                                    joint_states_topic="/arm_gen3/joint_states")
robot.reset_pose()
